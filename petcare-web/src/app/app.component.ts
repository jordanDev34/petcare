    ...
    this.vm$ = this.refresh$.pipe(
      ...
      switchMap(() =>
        this.healthService.getHealth().pipe(
          map((data) => ({ loading: false, data, error: null } satisfies Vm)),
          startWith({ loading: true, data: null, error: null } satisfies Vm),
          catchError((err) => {
            return of({ loading: false, data: null, error: 'API indisponible.' } satisfies Vm);
          })
        )
      ),
      // évite de relancer si plusieurs bindings/lectures
